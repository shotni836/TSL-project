...
import Rawmateriallist from '../../Raw-material-list/Rawmateriallist';
        ...
        const fetchData = async () => {
            ...
            try {
                if (tstmaterialid) {
                    ...
                    const mergedDataMap = data[3]?.reduce((acc, item) => {
                        const key = `${item.MaterialName}-${item.Grade}-${item.ManufacturerName}`;
                        if (acc[key]) {
                            acc[key].Batch += `,${item.Batch}`;
                        } else {
                            acc[key] = { ...item };
                        }
                        return acc;
                    ...
                    // setRawMaterialDetails(mergedData);

                    setMyIndex(data[3].length)
                                                                            ...
                                                                            {id4 === '226' || id4 === '243' || id4 === '260' || id4 === '650' || id4 === '215' || id4 === '208' ?
                                                                                testDetails.map((item, index) => (
                                                                                    <tr key={index}>
                                                                                        ...
                                                                                        {/* <td>{rawMaterialDetails[0]?.MaterialName || "-"}</td> */}
                                                                                        {rowSpans[index] > 0 && (
                                                                                            <td rowSpan={rowSpans[index]}>
                                                                                        ...
                                                                                        {/* <td>{id4 === '243' || id4 === '248' ? rawMaterialDetails.map((data, index) => index === rawMaterialDetails.length - 1 ? data?.Batch + ' ' : data?.Batch + ', ') || "-" : 'Grade : ' + rawMaterialDetails[0]?.Grade || "-"}</td> */}
                                                                                        {rowSpans[index] > 0 && (
                                                                                            <td rowSpan={rowSpans[index]}>
                                                                                                {item.test_name || "-"}
                                                                                            </td>
