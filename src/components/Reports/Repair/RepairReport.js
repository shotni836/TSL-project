...
function RepairReport() {
    ...
    for (let i = 0; i < pathSegments.length; i++) {
        if (pathSegments[i].startsWith('pm_test_run_id=')) {
            pm_project_id1 = pathSegments[i].substring('pm_test_run_id='.length);
        }
        if (pathSegments[i].startsWith('procsheet_id=')) {
            pm_processSheet_id1 = pathSegments[i].substring('procsheet_id='.length);
        }
        ...
        const fetchData = async () => {
            try {
                if (tstmaterialid) {
                    ...
                    const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetRepairReport?ProcessSheetID=${pm_processSheet_id1}&ProcessSheetTypeID=${1428}&TestRunId=${pm_project_id1}&TestId=${0}`);
                    ...
                    const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GETInstrumentDetailsByReportId?ReportId=${1428}`);
                ...
                try {
                    if (tstmaterialid) {
                        // const [id1, id2, id3, id4] = tstmaterialid.split('&');
                        // const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetInspectedByAcceptedByDetails?matid=${id3}&testId=${1}`);
