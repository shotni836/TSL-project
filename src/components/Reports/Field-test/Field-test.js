...
function FieldTestReport() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (tstmaterialid) {
                    const [id1, id2, id3, id4] = tstmaterialid.split('&');
                    const response = id4 != 987 ? await axios.post(`${Environment.BaseAPIURL}/api/User/GetFlexibilityTestReport?pm_procsheet_id=${id1}&testRunId=${id2}&rtype=${id3}&testid=${id4}`)
                    ...
                    id4 != 987 ? setTestDetails(data._ImpactTestReportDetails || []) : setTestDetails(data._CdTestDetails || [])
    ...
    const mergeRow = (testDetails) => {
        ...
        testDetails.forEach((item, index) => {
            const isSamePipeNo = (id4 === '294' ? item.acceptanceCriteria : id4 === '303' ? item.pipNo : id4 == 987 ? item.pipeNo : item.materialDescription) === currentPipeNo;
            ...
            if (isSamePipeNo) {
                ...
                currentPipeNo = (id4 === '294' ? item.acceptanceCriteria : id4 === '303' ? item.pipNo : id4 == 987 ? item.pipeNo : item.materialDescription);
                currentSpan = 1;
    ...
    const noOfRow = id4 === '284' || id4 === '304' ? 3 : id4 === '294' ? 23 : id4 === '303' ? 18 : 6;
