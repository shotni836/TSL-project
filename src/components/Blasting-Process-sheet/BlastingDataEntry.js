...
const BlastingDataEntry = () => {
    ...
    try {
      ...
      if (response.data[2].length > 0) {
        ...
        response.data[2].forEach((item) => {
          ...
          processedData[seqno].rejectReason = pm_test_value2;
  ...
  APIresponse.forEach(function (element, index) {
    ...
    if (
      ...
      const selectValues = dynamicField == "pHofPipeSurfaceAfterwaterwash" || dynamicField == 'pHofPipeSurfaceBeforewaterwash' || dynamicField == 'DegreeOfDustClass' || dynamicField == 'DegreeOfDustRating' ? [element.PM_Reqmnt_test_min, element.PM_Reqmnt_test_Max] : element?.pm_test_value?.split(/,\s*/)
    ...
    if (typeof parseFloat(newValue) === "number") {
      ...
      if (!params.colDef.field != 'Id' && !params.colDef.field != 'aslno' && !params.colDef.field != 'field_test' && !params.colDef.field != 'lab_test' && !params.colDef.field != 'is_repair' && !params.colDef.field != 'pm_ntc_id' && !params.colDef.field != 'remarks'
        ...
        if (
          ...
          // Update the remarks field to "Not Ok"

          if (["3oclock1", "3oclock2", "3oclock3", "6oclock1", "6oclock2", "6oclock3", "9oclock1", "9oclock2", "9oclock3", "12oclock1", "12oclock2", "12oclock3"].includes(colField)) {
            const currentRowIndex = params.node.rowIndex;
            const nextRowNode = api.getRowNode(currentRowIndex + 1);
          ...
          if (params.colDef.field == checkVar.current || checkVar.current == '') {
            ...
            if (colField != "ntc_reasons" && colField != "pm_reject_status" && colField != "field_test" && colField != "lab_test" && colField != "is_repair" && colField != "coat_status") {
              data.remarks = "Ok";
            }
            api.applyTransaction({ update: [data] });
          }
  ...
  const gridOptions = {
    ...
    getRowStyle: (params) => {
      if (params.data.pm_ntc_id && params.data.pm_ntc_id !== '') {
