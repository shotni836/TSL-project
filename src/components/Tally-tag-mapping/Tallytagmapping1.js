...
function Tallytagmapping() {
    ...
    const removeDuplicates = () => {
        ...
        const pipeCodesSet2 = new Set(modalData.map(entry => entry.pipeCode));
        // Find non-matching entries
        const nonMatchingEntries = {
            inSet1ButNotSet2: fileData.filter(entry => !pipeCodesSet2.has(entry['PIPE NO.'])),
            inSet2ButNotSet1: modalData.filter(entry => !pipeCodesSet1.has(entry.pipeCode))
        };
    ...
    const checkDuplicatePipeNumbers = () => {
        if (fileData.length > 0) {
            setErrorContent(
                <div className='ValidateModalDeleteConfirm'>
                    <h4>Validate</h4>
                    <table>
                        <thead>
                    ...
                    <div className='deleteFlexBox'>
                        ...
                        <button type="button" className="Cancelbtn" data-bs-dismiss="modal" onClick={(e) => { e.preventDefault(); setIsSubmitDisabled(true); }}>Cancel</button>
                    </div>
    ...
    const validateTallysheet = () => {
        ...
        axios.post(`${Environment.BaseAPIURL}/api/User/VildateTallySheetData`, dataArrays)
            .then(response => {
    ...
    const closeModal = (event) => {
        ...
        window.location.href = `/tallytagmappinglist?menuId=${menuId}`
    };
                                        ...
                                        <div className='PipeTallySheetDetails'>
                                            <form action="" className='row m-0'>
                                                ...
                                                {[
                                                    ...
                                                    { id: 'specification', label: 'Specification', value: headerData?.specgrade },
                                                    { id: 'poNo', label: 'PO.NO/LOA.NO', value: headerData?.PONo },
                                                            ...
                                                            {fileData.length > 0 && !isLoading && !errorMessage && (
                                                                <div className='Tallytagmappingtable'>
                                                                    <div className='NumberrowsSubmitFlexBox'>
                                                                        <p className='NumberrowsTxt'>Number of rows in the Excel file / Total Pipe Numbers : <b>{rowCount}</b></p>
                                                                        ...
                                                                        <div className='DownloadButton'>
                                                                            {isValidateButtonVisible && (
                                                                                <button type='button' style={{ display: isSubmitDisabled ? 'inline-block' : 'none' }} data-bs-toggle="modal" data-bs-target="#ValidatemodaL" onClick={() => {
                                                                                    validateTallysheet();
                                                                                    checkDuplicatePipeNumbers();
                                                                                }}>Validate</button>
                                                                            )}

                                                                            <div class="modal fade ValidateModalBox" id="ValidatemodaL" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="ValidatemodaLLabel" aria-hidden="true">
                                                                            ...
                                                                            <button type='button' data-bs-toggle="modal" data-bs-target="#exampleModal" disabled={isSubmitDisabled} style={{
                                                                                backgroundColor: isSubmitDisabled ? '#CCCCCC' : '#1353ad', // Change these colors as needed
                                                                        ...
                                                                        <div className="modal-dialog">
                                                                            <div className="modal-content">
                                                                                ...
                                                                                <div className="modal-body">
                                                                                    <h4>Great!</h4>
                                                                                    <p>Tally Sheet No. <b>{inputValue3}</b> is attached to <br /> Process Sheet.
                                                                                    </p>
                                                                                    <button onClick={(e) => { handleClick(); }} className="btn btn-danger">OK</button>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                                        {fileData.length > 0 && (
                                                                            <div className='NumberrowsSubmitFlexBox' style={{ border: 'none', margin: '10', padding: '0' }}>
                                                                                ...
                                                                                <label htmlFor="totalPipeLength" style={{ margin: '0' }}>Total Pipe Length:<span style={{ padding: '0.5em', color: '#518ada' }} id="totalPipeLength" readOnly>{totalPipeLength.toFixed(2)}</span></label>
                                                                                <div>
                                                                                    <button type='button' style={{ display: isSubmitDisabled ? 'inline-block' : 'none' }} data-bs-toggle="modal" data-bs-target="#ValidatemodaL" onClick={checkDuplicatePipeNumbers}>Validate</button>
